...
namespace CoreAnimation {
	...
	public:
		...
		std::optional<uint32_t> get_clip_id(std::string& clip_uid);
		AnimationClip& add_clip(std::string uid);
	private:
		std::unordered_map<std::string, uint32_t> clip_UID_to_ID;
	...
	public:
		...
		std::optional<uint32_t> get_animator_id(std::string& clip_uid);
		Animator& add_animator(std::string uid);
	private:
		std::unordered_map<std::string, uint32_t> animator_UID_to_ID;
