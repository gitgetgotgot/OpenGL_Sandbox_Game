...
void CoreUI::Image::_update_sprite_buffer(
	...
	//index_count = 4 vertices * 6 indices
	UI_RenderEntry& last_entry = render_queue.back();
	if (last_entry.type == UI_Component_Type::UI_SPRITE)
		last_entry.index_count += 6;
	else
		UI_RenderEntry& entry = render_queue.emplace_back(UI_Component_Type::UI_SPRITE, 6);
...
void CoreUI::Image::_update_render_data() {
	...
	float x = tr.global_pos.x;
	float y = tr.global_pos.y;
